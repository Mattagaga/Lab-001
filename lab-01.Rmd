...
_Answer is not available. This is because there are missing values in the data for 2021 (N/A). These are not numeric so can't be used in the mean._
...
_Edited above - when the function is set to true it 'ignores' n/a when calculated averages._
...
_If we were to draw a graph, it would not be normally distributed. It would instead be shifted towards the lower values. So the median is lower but a few attractions have significntly higher visitor numbers which pulls the mean up_
...
proportion_free <- split_by_nation %>%
  group_by(nation) %>%
  summarise(
    number_free = sum(admission == "Free"),
    total = n(),
    proportion_free = 100 * number_free / total
  )
show(proportion_free)
```
...
```{r Ex_n_a}
#Total increase
percentage_increase <- visitors %>%
  filter(!is.na(n_2021)) %>%
  summarise(
    change = sum(n_2022) - sum(n_2021, na.rm = TRUE),
    percentage_increase = 100 * change / sum(n_2021, na.rm = TRUE)
  )
show(percentage_increase)
```
```{r Ex_n_b}
#Increase by attraction in Scotland
indv_increase <- visitors %>%
  filter(region == "Scotland") %>%
  filter(!is.na(n_2021))

percent_change <- indv_increase %>%
  mutate(
    increase = n_2022 - n_2021,
    percent_change = 100 * increase / n_2021
  )
clean <- percent_change %>%
  select(attraction,admission, percent_change)
